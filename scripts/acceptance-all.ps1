...
#
# Aggregated acceptance runner for Sprints 1–6,8.
#
<#
.SYNOPSIS
  Aggregated acceptance runner for Sprints 1–6,8.
...
try {
  ...
  # Sprint 8
  Run 'Sprint 8 Acceptance' 'sprint8-acceptance.ps1' @{ ApiBase = $ApiBase }

  Write-Host "All Sprints (1–6,8) acceptance PASSED" -ForegroundColor Green
} catch {
  Write-Host $_ -ForegroundColor Red
  Write-Host "Aggregated acceptance FAILED" -ForegroundColor Red
  exit 1
