  ...
  return (
    <div className="relative" ref={menuRef}>
      <button className="hover:text-foreground" onClick={() => setOpen(o => !o)} aria-expanded={open} aria-haspopup="menu">
        {t('nav.cart')}{count > 0 ? <span className="ml-1 inline-flex items-center justify-center rounded-full bg-black text-white text-[10px] leading-none px-2 py-1">{count}</span> : null}
      </button>
      {open && (
        <div role="menu" className="absolute right-0 mt-2 w-72 rounded-md border border-border bg-white shadow-lg z-50">
          <div className="p-3">
            <div className="font-medium mb-2">{t('nav.cart')}</div>
            {items.length === 0 ? (
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="text-sm">{t('cart.subtotal')}: {(subtotalMinor / 100).toFixed(2)} {currency}</div>
              <Link href="/cart" className="btn btn-outline btn-sm">{t('nav.cart')}</Link>
            </div>
          </div>
        </div>
      )}
